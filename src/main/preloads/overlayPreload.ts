...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // Overlay-specific: mouse passthrough control
  overlayMouseEnter: () => {
    ipcRenderer.send(IPC.OVERLAY_MOUSE_ENTER)
  },

  overlayMouseLeave: () => {
    ipcRenderer.send(IPC.OVERLAY_MOUSE_LEAVE)
  },

  // Overlay-specific: drag to reposition
  overlayDrag: (deltaX: number, deltaY: number) => {
    ipcRenderer.send(IPC.OVERLAY_DRAG, { deltaX, deltaY })
  },
})
